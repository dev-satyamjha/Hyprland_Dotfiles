...
fetch_thumb() {
    ...
    magick "${THUMB}.png" -quality 50 -resize 256x256^ -gravity center -background none -extent 256x256 "${THUMB}.png"
    # Create blurred version
    magick "${THUMB}.png" -blur 200x7 -resize 256x256^ -gravity center -background none -extent 256x256 "${THUMB_BLURRED}.png"
...
--artist)
    ...
    if [ -z "$artist" ]; then
        ...
        echo "${artist:0:22}" #mit the output to 50 characters
    fi
    ;;
--position)
...
*)
    ...
    echo "Usage: $0 --title | --arturl | --artist | --position | --length | --album"
    exit 1
    ;;
esac
